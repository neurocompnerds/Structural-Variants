...
parser$add_argument("-v", "--vcf", type="character", required=TRUE,
    help="You must specify a VCF file to annotate")
parser$add_argument("-o", "--output", type="character", default="annotated",
    help="File path and output prefix for VCF and BED files [default %(default)s]")
...
vcf <- readVcf(paste(args$vcf), "hg38")
info(header(vcf)) = unique(as(rbind(as.data.frame(info(header(vcf))), data.frame(
	...
	Description=c("Simple event type annotation based purely on breakend position and orientation.", "Difference in length between REF and ALT alleles."))), "DataFrame"))
...
writeVcf(vcf, paste(args$output, "vcf", sep = ".")) # generated by example/gridss.sh
...
write.table(simplebed, paste(args$output, "bed", sep = "."), quote=FALSE, sep='\t', row.names=FALSE, col.names=FALSE)
