...
`GRIDSS`, `Parliament2`, `MEI/retroseq`, `clinsv`, `wisecondorx`

Things that definitely *don't* work:

`delly` (use Parliament2 instead), `smoove`

## Usage notes
Most of these pipelines are described in our Google Doc protocols.
...
GRIDSS has multiple stages associated with it.  You can execute the entire pipeline from the command line using the `GRIDSS/gridss_launcher.sh` script.
...
The WISECONDORX pipeline is for low coverage short-read genome sequencing data and can be executed using the `wisecondorx/wisecondorx_launcher.sh` script.  Note that you will need to set up a conda environment for this to work, see instructions for how to do this in the specific `wisecondorx/README.wisecondorx.md` file.
